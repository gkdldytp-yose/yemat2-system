...
def _is_logistics_manager():
    role = (session.get('user') or {}).get('role', 'readonly')
    return role in ('logistics',)
...
def _ledger_workplaces():
    return [wp for wp in WORKPLACES if wp != '기타']
...
def _transfer_logistics_stock_to_workplace(cursor, material_id, workplace, qty):
    ...
    if available < qty_need:
        raise ValueError('물류 위치 재고가 부족합니다.')
...
def _register_export_request_row(cursor, workplace, req_user, material_id, lot_id, quantity, reason, reason_detail, note):
    ...
    cursor.execute('SELECT id, code, name, unit, current_stock FROM materials WHERE id = ?', (material_id,))
    mat = cursor.fetchone()
    if not mat:
    ...
    cursor.execute(
        '''
        SELECT id, material_id, lot, COALESCE(current_quantity, quantity, 0) AS current_quantity
        FROM material_lots
        WHERE id = ? AND material_id = ? AND COALESCE(is_disposed, 0) = 0
        ''',
        (lot_id, material_id),
    )
    lot = cursor.fetchone()
    ...
    mat_qty = float(mat['current_stock'] or 0)
    if lot_qty < quantity:
    ...
    cursor.execute(
        'UPDATE material_lots SET current_quantity = current_quantity - ? WHERE id = ?',
        (quantity, lot_id),
    )
    cursor.execute(
    ...
    if not workplace_location_id or not logistics_location_id:
        raise ValueError('재고 위치 정보를 찾을 수 없습니다.')
    _move_lot_balance_between_locations(cursor, lot_id, workplace_location_id, logistics_location_id, quantity)
    cursor.execute(
        '''
        INSERT INTO material_lot_logs (material_lot_id, material_id, action, quantity, note)
        VALUES (?, ?, 'export_request', ?, ?)
        ''',
        (lot_id, material_id, quantity, f'{workplace} 반출 ({reason})'),
    )

    pool_code = _pool_code_from_row(mat)
    if reason == '불량':
        _increase_logistics_defect_stock(cursor, pool_code, mat['name'], mat['unit'], quantity, req_user)
    else:
        _increase_logistics_stock(cursor, pool_code, mat['name'], mat['unit'], quantity, req_user)
    ...
    cursor.execute(
        ...
        (material_id, material_code, material_name, unit, requester_workplace, requested_quantity, approved_quantity,
         request_type, reason, reason_detail, material_lot_id, status, note, requested_by, processed_by, processed_at, process_note)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'RETURN', ?, ?, ?, '완료', ?, ?, ?, CURRENT_TIMESTAMP, ?)
        ''',
        (
            material_id,
            pool_code,
...
def add_supplier():
    ...
    # 발주 관리 페이지로 리다이렉트 (업체 관리 모달에서 등록하므로)
...
def materials():
    ...
    logistics_stock_by_code = {}
    cursor.execute('SELECT material_code, COALESCE(quantity, 0) AS quantity FROM logistics_stocks')
    for srow in cursor.fetchall():
        logistics_stock_by_code[(srow['material_code'] or '').strip()] = float(srow['quantity'] or 0)

    materials = []
    for row in material_rows:
        item = dict(row)
        pool_code = _pool_code_from_row(item)
        workplace_stock = float(item.get('current_stock') or 0)
        logistics_stock = float(logistics_stock_by_code.get(pool_code, 0) or 0)
    ...
    if is_logistics:
        ...
        cursor.execute(
            ...
            SELECT id, code, name, unit
            FROM materials
            WHERE (workplace = ? OR workplace = ? OR workplace IS NULL)
            ORDER BY category, name
            ''',
            (workplace, SHARED_WORKPLACE),
        )
    ...
    # 5. 작업장 로트(반출 요청 시 로트 선택용)
    material_lots_by_material = {}
    if not is_logistics:
        cursor.execute(
            '''
            SELECT ml.id, ml.material_id, ml.lot, ml.receiving_date, COALESCE(ml.current_quantity, ml.quantity, 0) AS current_quantity
            FROM material_lots ml
            JOIN materials m ON m.id = ml.material_id
            WHERE COALESCE(ml.is_disposed, 0) = 0
              AND COALESCE(ml.current_quantity, ml.quantity, 0) > 0
...
def move_material_workplace(material_id):
    ...
    try:
        move_quantity = float(request.form.get('move_quantity') or 0)
    except ValueError:
        move_quantity = 0
    valid_targets = set(WORKPLACES) | {LOGISTICS_WORKPLACE}
    if target_workplace not in valid_targets:
        return "<script>alert('이동할 작업장을 다시 선택해주세요.'); history.back();</script>"
    if move_quantity <= 0:
        return "<script>alert('이동 수량을 확인해주세요.'); history.back();</script>"
    ...
    try:
        ...
        current_stock = float(before['current_stock'] or 0)
        if move_quantity > current_stock:
            return "<script>alert('이동 수량이 현재 재고보다 큽니다.'); history.back();</script>"
        ...
        if source_workplace == final_workplace:
            return "<script>alert('같은 작업장으로는 이동할 수 없습니다.'); history.back();</script>"

        username = session.get('user', {}).get('name') or session.get('user', {}).get('username')

        # 물류 이동은 수량 이동(작업장 재고 차감 + 물류 재고 증가)
        if final_workplace == LOGISTICS_WORKPLACE:
            cursor.execute(
                'UPDATE materials SET current_stock = current_stock - ? WHERE id = ? AND current_stock >= ?',
                (move_quantity, material_id, move_quantity),
            )
            if cursor.rowcount == 0:
                return "<script>alert('재고 이동 처리 중 오류가 발생했습니다.'); history.back();</script>"

            _increase_logistics_stock(
                cursor,
                _pool_code_from_row(before),
                before['name'],
                before['unit'],
                move_quantity,
                username,
            ...
            cursor.execute(
                ...
                VALUES (?, 'MOVE_TO_LOGISTICS', ?, ?, ?, datetime('now'))
                ''',
                (
                    material_id,
                    move_quantity,
        ...
        # 현재 데이터 구조상 타 작업장 이동은 전체 수량 이동만 지원
        if abs(move_quantity - current_stock) > 1e-9:
            return "<script>alert('타 작업장 이동은 현재고 전체 수량으로만 가능합니다.'); history.back();</script>"

        cursor.execute('UPDATE materials SET workplace = ? WHERE id = ?', (final_workplace, material_id))
        cursor.execute(
            '''
            INSERT INTO material_history (material_id, type, quantity, reason, note, created_at)
            VALUES (?, 'MOVE_WORKPLACE', ?, ?, ?, datetime('now'))
            ''',
            (
                material_id,
                move_quantity,
...
def material_detail(material_id):
    ...
    try:
        ...
        if not material:
            return jsonify({'ok': False, 'message': '부자재를 찾을 수 없습니다.'}), 404
...
def add_material_lot():
    ...
    try:
        ...
        conn.rollback()
        return jsonify({'ok': False, 'message': '로트 추가 중 오류가 발생했습니다.'}), 500
    finally:
        conn.close()
...
def update_material_lot(lot_id):
    ...
    try:
        ...
        conn.rollback()
        return jsonify({'ok': False, 'message': '로트 수정 중 오류가 발생했습니다.'}), 500
    finally:
        conn.close()
...
def raw_materials():
    ...
    for i in range(6):
        if today.month - i > 0:
            m = today.replace(month=today.month - i)
        else:
            m = today.replace(year=today.year - 1, month=12 + (today.month - i))
...
def _render_purchase_orders_page(
    ...
    if show_logistics_hub:
        ...
        cursor.execute(
            '''
            SELECT MIN(target_date) AS focus_date
            FROM (
                SELECT expected_delivery_date AS target_date
                FROM purchase_requests pr
                ...
                UNION ALL
                SELECT substr(requested_at, 1, 10) AS target_date
        ...
        logistics_focus_date = (focus_row['focus_date'] if focus_row else None) or None
...
def logistics_ledger():
    ...
    try:
        ...
        materials_where = ''
        if q:
            materials_where = 'WHERE (m.code LIKE ? OR m.name LIKE ? OR COALESCE(s.name, \'\') LIKE ?)'
            like_q = f'%{q}%'
            materials_params.extend([like_q, like_q, like_q])

        cursor.execute(
            f'''
            SELECT m.id, m.code, m.name, m.unit, m.unit_price, COALESCE(s.name, '-') AS supplier_name
            FROM materials m
            LEFT JOIN suppliers s ON s.id = m.supplier_id
            {materials_where}
            ORDER BY m.name
            ''',
            tuple(materials_params),
        )
        ...
        raw_where = ''
        if q:
            raw_where = "WHERE (COALESCE(NULLIF(TRIM(code), ''), printf('RM%05d', id)) LIKE ? OR name LIKE ?)"
            like_q = f'%{q}%'
            raw_params.extend([like_q, like_q])
        cursor.execute(
            f'''
...
def complete_issue_request(req_id):
    ...
    try:
        ...
        cursor.execute(
            'SELECT quantity FROM logistics_stocks WHERE material_code = ?',
            (req_row['material_code'],),
        )
        stock_row = cursor.fetchone()
        logistics_qty = float(stock_row['quantity'] or 0) if stock_row else 0.0
        ...
        cursor.execute(
            '''
            UPDATE logistics_stocks
            SET quantity = quantity - ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP
            WHERE material_code = ?
            ''',
            (moved_qty, session.get('user', {}).get('name'), req_row['material_code']),
        )
        cursor.execute(
            '''
            UPDATE materials
